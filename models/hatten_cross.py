...
class CriteriaModel(nn.Module):
    ...
    def forward(self, incl_emb, incl_mask, excl_emb, excl_mask, dense_embeddings, short_onehot, sparse_embeddings):
        ...
        # assert not torch.isnan(concatenated).any(), "NaN in concatenated"
...
def train(params):
    ...
    np.save("mcnemar/results/y_test_dcn.npy", y_test)
    np.save("mcnemar/results/y_pred_dcn.npy", y_pred_probs)
...
def run_hyperparameter_test():
    ...
    space = {
        ...
        "batch_size": 64,
        #"weight_decay": hp.uniform('weight_decay', 4e-5, 6e-5)
        "weight_decay": 1e-5
